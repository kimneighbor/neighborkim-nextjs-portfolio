...
export default function ProjectItem({ data }: any) {
  ...
  return (
    ...
    <div className="xl:w-1/4 md:w-1/2 p-3 slide-up">
      <div className="rounded-lg h-full item-bg item-shadow">
        ...
        <Link href={imgurl}>
          ...
          <div className="px-5 pt-5">
            <h3 className="tracking-widest dark:text-blue-400 text-blue-600 text-xs font-medium title-font mb-1">
              {subtitle}
            </h3>
        ...
        <div className="px-5 pb-5">
          ...
          <div className="flex flex-wrap items-start mb-3">
            {tags.map((aTag: any) => (
              <h6
                className={`text-xs px-2 py-0.5 mr-2 mb-2 rounded-md ${getColorByTagName(
                  aTag.name
                )}`}
                key={aTag.id}
              >
